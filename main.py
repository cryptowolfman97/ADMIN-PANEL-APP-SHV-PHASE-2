...
# ── Final nav redesign + anti-flicker safeguard ─────────────────────────────
from kivy.uix.scrollview import ScrollView as _BottomNavScrollView
...
NAV_DOCK_BG = '#041018'
NAV_DOCK_BORDER = '#102233'
NAV_DOCK_ACTIVE = '#5ed8ca'
NAV_DOCK_IDLE = '#0f1c27'
NAV_DOCK_ACTIVE_TXT = '#051317'
NAV_DOCK_IDLE_TXT = '#d7e8ef'

def _update_rect(rect, widget, radius=None):
    rect.pos = widget.pos
...
def _make_bottom_nav(screen_instance, active_name=''):
    outer = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(78), padding=[0, dp(2), 0, 0])

    sep = Widget(size_hint_y=None, height=dp(1))
    with sep.canvas.before:
        Color(rgba=get_color_from_hex('#0d2330'))
        sep_rect = RoundedRectangle(pos=sep.pos, size=sep.size)
    sep.bind(pos=lambda *_: _update_rect(sep_rect, sep), size=lambda *_: _update_rect(sep_rect, sep))
    outer.add_widget(sep)

    holder = BoxLayout(orientation='vertical', size_hint_y=None, height=dp(76), padding=[dp(6), dp(6), dp(6), dp(8)])
    with holder.canvas.before:
        Color(rgba=get_color_from_hex(NAV_DOCK_BG))
        holder_bg = RoundedRectangle(pos=holder.pos, size=holder.size, radius=[dp(18)])
        Color(rgba=get_color_from_hex(NAV_DOCK_BORDER))
        holder_outline = RoundedRectangle(pos=(holder.x, holder.y), size=holder.size, radius=[dp(18)])
    holder.bind(pos=lambda *_: (_update_rect(holder_bg, holder, dp(18)), _update_rect(holder_outline, holder, dp(18))),
                size=lambda *_: (_update_rect(holder_bg, holder, dp(18)), _update_rect(holder_outline, holder, dp(18))))

    scroll = _BottomNavScrollView(do_scroll_y=False, do_scroll_x=True, bar_width=0, scroll_type=['content'])
    row = BoxLayout(orientation='horizontal', spacing=dp(6), size_hint_x=None, height=dp(58), padding=[dp(2), 0, dp(2), 0])
    row.bind(minimum_width=row.setter('width'))

    def _nav(target):
        try:
            if getattr(screen_instance, 'manager', None) is not None:
                screen_instance.manager.current = target
        except Exception:
            pass

    for label, target in _NAV_ITEMS:
        active = (target == active_name) or (active_name == 'accounts' and target == 'accounts' and getattr(screen_instance, 'name', '') in ('accounts', 'emergency_vault', 'cloud_users', 'supabase_control'))
        btn = RoundedButton(
            text=label,
            bg_hex=NAV_DOCK_ACTIVE if active else NAV_DOCK_IDLE,
            text_color=get_color_from_hex(NAV_DOCK_ACTIVE_TXT if active else NAV_DOCK_IDLE_TXT),
            radius=14,
            size_hint=(None, None),
            width=dp(122 if len(label) > 8 else 110),
            height=dp(54),
            font_size='15sp',
            bold=True,
        )
        btn.bind(on_release=lambda *_, t=target: _nav(t))
        row.add_widget(btn)

    scroll.add_widget(row)
    holder.add_widget(scroll)
    outer.add_widget(holder)
    return outer

def _schedule_admin_postprocess_once(target, delay=0.03):
    try:
        if target is None:
            return
        ev = getattr(target, '_shv_postprocess_event', None)
        if ev is not None:
            try:
                ev.cancel()
            except Exception:
        ...
        def _run(*_):
            try:
                ...
                try:
                    target._shv_postprocess_event = None
                except Exception:
                    pass
        target._shv_postprocess_event = Clock.schedule_once(_run, delay)
    except Exception:
        pass

_prev_budget_shell_on_pre_enter_final = getattr(BudgetManagerShellScreen, 'on_pre_enter', None)
def _budget_shell_on_pre_enter_final(self, *args, **kwargs):
    result = _prev_budget_shell_on_pre_enter_final(self, *args, **kwargs) if callable(_prev_budget_shell_on_pre_enter_final) else None
    _schedule_admin_postprocess_once(self, 0.04)
    return result
BudgetManagerShellScreen.on_pre_enter = _budget_shell_on_pre_enter_final

_prev_ssp_shell_on_pre_enter_final = getattr(StrategySuiteManagerShellScreen, 'on_pre_enter', None)
def _ssp_shell_on_pre_enter_final(self, *args, **kwargs):
    result = _prev_ssp_shell_on_pre_enter_final(self, *args, **kwargs) if callable(_prev_ssp_shell_on_pre_enter_final) else None
    _schedule_admin_postprocess_once(self, 0.04)
    return result
StrategySuiteManagerShellScreen.on_pre_enter = _ssp_shell_on_pre_enter_final

_prev_open_product_manager_final_nav = SHVertexAdminPanelApp.open_product_manager
def _open_product_manager_final_nav(self, screen_name, tab=None, subtab=None):
    result = _prev_open_product_manager_final_nav(self, screen_name, tab, subtab)
    _schedule_admin_postprocess_once(getattr(self, 'root', None), 0.03)
    return result
SHVertexAdminPanelApp.open_product_manager = _open_product_manager_final_nav

_prev_build_final_nav = SHVertexAdminPanelApp.build
def _build_final_nav(self, *args, **kwargs):
    root = _prev_build_final_nav(self, *args, **kwargs)
    try:
        if not getattr(root, '_shv_postprocess_bound', False):
            root.bind(current=lambda *_: _schedule_admin_postprocess_once(root, 0.02))
            root._shv_postprocess_bound = True
        _schedule_admin_postprocess_once(root, 0.02)
    except Exception:
        pass
    return root
SHVertexAdminPanelApp.build = _build_final_nav
