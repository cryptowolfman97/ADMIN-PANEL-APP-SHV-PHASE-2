...
def _premium_admin_home_refresh_ui(self, *_):
    ...
    def _toggle_dashboard(*_):
        next_state = not bool(getattr(self, '_product_dashboard_collapsed', False))
        current_dash = getattr(self, '_product_dashboard_card', None)
        if not Animation or current_dash is None:
            self._product_dashboard_collapsed = next_state
            self.refresh_ui()
            return
        def _finish(*__):
            self._product_dashboard_collapsed = next_state
            self._product_dashboard_animating = True
            self.refresh_ui()
        try:
            Animation.cancel_all(current_dash)
            anim = Animation(opacity=0.0, duration=0.14, t='out_quad')
            anim.bind(on_complete=lambda *_args: _finish())
            anim.start(current_dash)
        except Exception:
            self._product_dashboard_collapsed = next_state
            self.refresh_ui()
    ...
    if self._product_dashboard_collapsed:
        ...
        for summary, accent, screen_name in product_rows:
            ...
            card.add_widget(open_btn)
            dash.add_widget(card)
    self._product_dashboard_card = dash
    if getattr(self, '_product_dashboard_animating', False):
        dash.opacity = 0.0
    self.root_box.add_widget(dash)
    if getattr(self, '_product_dashboard_animating', False):
        try:
            Animation.cancel_all(dash)
            Animation(opacity=1.0, duration=0.22, t='out_quad').start(dash)
            Clock.schedule_once(lambda *_: _animate_children(dash), 0.02)
        except Exception:
            pass
        self._product_dashboard_animating = False
